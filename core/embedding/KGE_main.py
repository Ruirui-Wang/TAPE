...
# from embedding.KGE_subskep import KGEModel
import argparse
import time
import torch
...
def KGE_dataset(data, split):
    ...
    pos_tuples = torch.stack((source_nodes, torch.full_like(source_nodes, 
            default_relation), target_nodes), dim=1)
    ...
    source_nodes = data.x[edge_index[0]]
    target_nodes = data.x[edge_index[1]]
    neg_tuples = torch.stack((source_nodes, torch.full_like(source_nodes, 
            default_relation), target_nodes), dim=1)
...
if __name__ == "__main__":
    ...
    for run_id in range(args.repeat):
        ...
        model = KGEModel(cfg.model.model, data.num_nodes, 1, feature_dim, criterion='mrl', gamma=cfg.model.gamma)
